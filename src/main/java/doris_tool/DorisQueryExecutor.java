...
public class DorisQueryExecutor {
    ...
    public static void executeQuery(String code,String date) {
        ...
        executorService.submit(() -> {
            ...
            String queryStart="SELECT * FROM  ticket  WHERE  order_code ='"+code+"' AND  order_date ='"+date+"' AND  tran_id IS NOT NULL order by order_time ASC limit  1";
            String queryEnd="SELECT * FROM  ticket  WHERE  order_code ='"+code+"' AND  order_date ='"+date+"' AND  tran_id IS NOT NULL order by order_time desc limit  1";
            ...
            try(Connection connection = DatabaseConnectionPoolNew.getConnection();
                ...
                // 构造插入数据的 SQL 语句
                String sql = "INSERT INTO ticket_up_down (" +
                        "order_code," +
                        " order_date," +
                        "start_price," +
                        "end_price," +
